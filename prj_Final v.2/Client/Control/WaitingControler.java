...
import Thread.RoomInfoRecieveThread;
...
public class WaitingControler {
		...
		public void actionPerformed(ActionEvent event){
			if(event.getSource() == view.getMkr_bt()) {
				...
				if(w.getRoom_num() < 4) {
					...
					mv = new MakeRoomView();
					// 거기서 정보 만들기
					mc = new MakeRoomControler(app,mv,WaitingControler.this,ic,s3,ID,avatar);
					mc.buttonHandler();
					// 생성된 방이 4개 이상일때
				} else {
	...
	public void setUserInfo(Socket s1, Socket s2, Socket s3, String ID, String avatar) {
		...
		mrrt = new RoomInfoRecieveThread(s3, view, ic, w);
		mrrt.start();
	}
